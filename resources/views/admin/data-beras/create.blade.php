                    ...
                    <form action="{{ route('admin.data-beras.store') }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <x-text-input id="produksi_ton" class="block mt-1 w-full input-hitung" type="number" step="0.01" name="produksi_ton" :value="old('produksi_ton')" required />
                                <x-input-error :messages="$errors->get('produksi_ton')" class="mt-2" />
                            </div>

                            <div>
                                <x-input-label for="stok_awal_ton" :value="__('Stok Awal (Ton)')" />
                                <x-text-input id="stok_awal_ton" class="block mt-1 w-full input-hitung" type="number" step="0.01" name="stok_awal_ton" :value="old('stok_awal_ton', 0)" />
                                <x-input-error :messages="$errors->get('stok_awal_ton')" class="mt-2" />
                            </div>

                            <div>
                                <x-input-label for="konsumsi_ton" :value="__('Konsumsi (Ton)')" />
                                <x-text-input id="konsumsi_ton" class="block mt-1 w-full input-hitung" type="number" step="0.01" name="konsumsi_ton" :value="old('konsumsi_ton')" required />
                                <x-input-error :messages="$errors->get('konsumsi_ton')" class="mt-2" />
                            </div>

                            <div>
                                <x-input-label for="impor_ton" :value="__('Impor (Ton)')" />
                                <x-text-input id="impor_ton" class="block mt-1 w-full input-hitung" type="number" step="0.01" name="impor_ton" :value="old('impor_ton', 0)" />
                            </div>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea id="catatan" name="catatan" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('catatan') }}</textarea>
                            </div>
                        </div>
